...
describe("Logger", () => {
  ...
  test("formatLogMessage formats messages correctly", () => {
    ...
    const message = logger.formatLogMessage({
      type: LogLevel.ERROR,
      message: "test message",
      args: [{ detail: "some detail" }],
    });
  ...
  test("NamespacedLogger adds namespace prefix to messages", () => {
    ...
    expect(mockLogger.messages[0].message).toBe("test message");
    expect(mockLogger.messages[0].namespace).toBe("TestModule");
  });
  ...
  test("Multiple NamespacedLoggers can use the same underlying logger", () => {
    ...
    expect(mockLogger.messages[0].namespace).toBe("ModuleA");
    expect(mockLogger.messages[1].namespace).toBe("ModuleB");
    expect(mockLogger.messages[0].message).toBe("message from A");
    expect(mockLogger.messages[1].message).toBe("message from B");
  });
  ...
  test("NamespacedLogger inherits log level changes from underlying logger", () => {
    ...
    expect(mockLogger.messages.length).toBe(1);
    expect(mockLogger.messages[0].namespace).toBe("TestModule");
    expect(mockLogger.messages[0].message).toBe("warning message");
  });
